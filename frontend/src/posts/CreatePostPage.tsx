...
import { Loader2, AlertCircle, Plus, X, Upload, Image as ImageIcon, Video, FolderOpen } from 'lucide-react';
import { useNavigate } from '@tanstack/react-router';
import { validateCaption, validateImageUrls, validateVideoUrl } from '../validation/validators';
...
export default function CreatePostPage() {
  ...
  const [mediaEntries, setMediaEntries] = useState<MediaEntry[]>([{ url: '', isDataUrl: false }]);
  ...
  const { processMediaFiles, processClipboardItems, uploadProgress } = useMediaUpload();
  ...
  useEffect(() => {
    const handlePaste = async (e: ClipboardEvent) => {
      ...
      try {
        ...
        if (results.length > 0) {
          const successfulUrls = results.filter(r => !r.error).map(r => r.url);
          if (successfulUrls.length > 0) {
            setMediaEntries(prev => {
              const filtered = prev.filter(e => e.url.trim() !== '');
              const newEntries: MediaEntry[] = successfulUrls.map(url => ({ url, isDataUrl: true, previewType: 'image' as const }));
              return [...filtered, ...newEntries].slice(0, 10);
            });
            toast.success(`${successfulUrls.length} image(s) pasted successfully`);
          }
        ...
        toast.error(err instanceof Error ? err.message : 'Failed to paste media');
      }
    };
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to process dropped files');
    } finally {
  ...
  const handleFilePickerChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to process selected files');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (contentType === 'reel') {
      ...
      const urlOnlyEntries = filteredEntries.filter(e => !e.isDataUrl).map(e => e.url);
    ...
    try {
      const backendContentType = contentType === 'reel' ? ContentType.reel : ContentType.post;
  ...
  return (
    <div className="space-y-6 p-4 pb-24 md:p-6 md:pb-6">
      <h1 className="text-2xl font-bold">Create {contentType === 'reel' ? 'Reel' : 'Post'}</h1>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup
                value={contentType}
                onValueChange={(value) => setContentType(value as 'post' | 'reel')}
                className="flex gap-4"
              >
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="post" id="post" />
                  <Label htmlFor="post" className="flex items-center gap-2 cursor-pointer font-normal">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="reel" className="flex items-center gap-2 cursor-pointer font-normal">
            ...
            <div className="space-y-3">
              ...
              {mediaEntries.some(e => e.isDataUrl || e.previewType === 'video') && (
                <div className="grid grid-cols-3 gap-2">
                  {mediaEntries.map((entry, index) =>
                    (entry.isDataUrl || entry.previewType === 'video') && entry.url ? (
                      <div key={index} className="relative group rounded-md overflow-hidden bg-muted aspect-square">
