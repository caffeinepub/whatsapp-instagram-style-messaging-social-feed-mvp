...
import type { UserProfile, Post, Message, ConversationId, UserSearchResult, PublicUserProfile, ProfilePrivacy, ContentType, PostId, CommentId, Group, GroupId, GroupMessage, MediaAsset } from '../backend';
  ...
  return useQuery<UserProfile | null>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.error('[useGetUserProfile] Error fetching user profile:', error);
        return null;
      }
    },
...
export function useSignUp() {
  ...
  return useMutation({
    mutationFn: async ({ username, displayName, privacy }: { username: string; displayName: string; privacy: ProfilePrivacy }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useSearchUsers(searchTerm: string) {
  ...
  return useQuery<UserSearchResult[]>({
    ...
    enabled: !!actor && !actorFetching && searchTerm.trim().length > 0 && !!identity,
    retry: false,
  });
}
...
export function useCreatePost() {
  ...
  return useMutation({
    mutationFn: async ({ caption, media, contentType }: { caption: string; media: MediaAsset[]; contentType: ContentType }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useGetConversationDetails(conversationId: bigint) {
  ...
  return useQuery<{ otherUserName: string; lastMessage?: string; otherUserId?: string }>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.error('[useGetConversationDetails] Error fetching profile:', error);
        return {
...
export function useInviteToGroup() {
  ...
  return useMutation({
    mutationFn: async ({ groupId, target }: { groupId: GroupId; target: string }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useSendGroupMessage() {
  ...
  return useMutation({
    mutationFn: async ({ groupId, content }: { groupId: GroupId; content: string }) => {
      if (!actor) throw new Error('Actor not available');
