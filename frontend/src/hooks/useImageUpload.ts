...
export function useImageUpload() {
  ...
  const processImageFiles = async (files: FileList | File[]): Promise<ImageUploadResult[]> => {
  ...
  const processClipboardItems = async (items: DataTransferItemList): Promise<ImageUploadResult[]> => {
    const results: ImageUploadResult[] = [];
