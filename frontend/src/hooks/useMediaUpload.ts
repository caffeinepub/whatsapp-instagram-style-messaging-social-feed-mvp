...
export function useMediaUpload() {
  ...
  const fileToDataUrl = (file: File, onProgress?: (pct: number) => void): Promise<string> => {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();

      const timeout = setTimeout(() => {
        reader.abort();
        reject(new Error('File reading timed out. The file may be too large to process.'));
  ...
  const processMediaFile = async (file: File): Promise<MediaUploadResult> => {
    ...
    if (!isImage && !isVideo) {
      return { url: '', isDataUrl: false, fileType: 'image', error: 'File must be an image or video' };
    }

    if (isImage && file.size > IMAGE_MAX_SIZE) {
      return { url: '', isDataUrl: false, fileType: 'image', error: 'Image files must be under 100MB' };
    }

    if (isVideo && file.size > VIDEO_MAX_SIZE) {
      return { url: '', isDataUrl: false, fileType: 'video', error: 'Video files must be under 2500MB (2.5GB)' };
    }
    ...
    if (isVideo) {
      try {
        ...
        return {
          ...
          isDataUrl: false,
          fileType: 'video',
          error: err instanceof Error ? err.message : 'Failed to process video file',
        };
      }
    }
    ...
    try {
      setUploadProgress(0);
      const dataUrl = await fileToDataUrl(file, (pct) => setUploadProgress(pct));
      setUploadProgress(100);
      return {
        url: dataUrl,
        isDataUrl: true,
  ...
  const processMediaFiles = async (files: FileList | File[]): Promise<MediaUploadResult[]> => {
  ...
  const processImageFile = async (file: File): Promise<{ url: string; error?: string }> => {
    const result = await processMediaFile(file);
    return { url: result.url, error: result.error };
  };

  const processImageFiles = async (files: FileList | File[]): Promise<{ url: string; error?: string }[]> => {
    const fileArray = Array.from(files);
    return Promise.all(fileArray.map(processImageFile));
  };

  const processClipboardItems = async (items: DataTransferItemList): Promise<{ url: string; error?: string }[]> => {
