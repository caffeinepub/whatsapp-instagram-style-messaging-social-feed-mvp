...
import type { GroupId, GroupMessage } from "../backend";
...
export default function GroupChatPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Please log in to view group chats.
        </p>
  ...
  const renderedItems: Array<
    { type: "date"; label: string } | { type: "message"; msg: GroupMessage }
  > = [];
  let lastDateLabel = "";
        ...
        {!messagesLoading && !messagesError && renderedItems.length > 0 && (
          <div className="space-y-2 pb-2">
            {renderedItems.map((item) => {
              if (item.type === "date") {
                return (
                  <DateDivider key={`date-${item.label}`} label={item.label} />
                );
              }
              const isOwn = item.msg.sender.toString() === currentUserId;
              return (
                <MessageBubble
                  key={`${item.msg.sender.toString()}-${item.msg.timestamp.toString()}`}
                  message={item.msg}
                  isOwn={isOwn}
                />
              );
