...
export default function PostCard({ post }: PostCardProps) {
  ...
  const hasLiked = currentUserId
    ...
    : false;
  const isSaved =
    currentUserProfile?.savedPosts.some(
            ...
            <div className="relative">
              {mediaItems.length === 1 ? (
                renderMediaItem(
                  mediaItems[0].url,
                  0,
                  isReel || mediaItems[0].url.startsWith("data:video"),
                )
              ) : (
                <Carousel className="w-full">
                  <CarouselContent>
                    {mediaItems.map((asset) => (
                      <CarouselItem key={asset.url}>
                        {renderMediaItem(
                          asset.url,
                          0,
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this {isReel ? "reel" : "post"}?
